...
| Phương thức | Endpoint | Chức năng (Ý nghĩa) | Quyền (Role) | Body / Params / Form-Data |
| :--- | :--- | :--- | :--- | :--- |
| **POST** | `/upload` | HR đẩy CV mới. Tự động test trùng lặp SĐT/Email (chặn dọn rác bằng cờ 409). | `HR`, `ADMIN` | Form-Data: <br/>- File: `cvFile`<br/>- Text: `name`, `email`, `phone`, `role_code`, `school_code`, `demo_link`<br/>- **`forceCreate`** (boolean): Nếu true, bỏ qua check trùng lặp.<br/><br/>👉 **Response:** `201` (Success) hoặc `409 Conflict` (Kèm Payload ứng viên trùng lặp). |
